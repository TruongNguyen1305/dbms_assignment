...
export const createUserCart = createAsyncThunk('cart/createUserCart', async ({ products, id }, { getState, rejectWithValue }) => {
  try {
    ...
    // GET THE USER'S CART
    let res = await axios.get(`/api/cart/${id}`, config)

    // CHECK IF THE USER HAS A CART IN DB
    if ((res.data === null)) {
      // IF NO CART, CREATE CART
      res = await axios.post(`/api/cart/`, { products, id }, config)
    } else {
...
export const updateUserCart = createAsyncThunk('cart/updateUserCart', async ({ products, id }, { getState, rejectWithValue }) => {
  try {
    const userToken = localStorage.getItem('userToken')
      ? localStorage.getItem('userToken')
      : null
    ...
    await axios.get(`/api/cart/${id}`, config)
    //  UPDATE USER'S CART
    let res = await axios.put(`/api/cart/${id}`, { products }, config)
    let data = res.data
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addToCart: (state, action) => {
      ...
      if (state.cartItems.map(item => item.id).includes(action.payload.product.id)) {
        for (let i = 0; i < state.cartItems.length; i++) {
          // GET THE INDEX OF THE ITEM
          if (state.cartItems[i].id === action.payload.product.id) {
