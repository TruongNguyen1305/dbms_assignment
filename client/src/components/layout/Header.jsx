...
const Header = () => {
  ...
  useEffect(() => {
    if (userInfo) {
      dispatch(createUserCart({ products: cartItems, id: userInfo.id }));
  ...
  useEffect(() => {
    if (userInfo) {
      dispatch(updateUserCart({ products: [...cartItems], id: userInfo.id }));
    }
    // eslint-disable-next-line
  }, [userInfo, cartItems]);
  ...
  useEffect(() => {
    if (userInfo) {
      dispatch(getUserAddress({ user: userInfo.id }));
      dispatch(getUserOrder({ user: userInfo.id }));
    }
    // eslint-disable-next-line
  }, [userInfo]);
